...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{4}[][gros œufs]
		\ingredient{1}[tasse][lait entier]
		\ingredient{2}[c. à soupe][sucre blanc]
		\ingredient{1}[c. à café][extrait de vanille]
		\ingredient{1}[c. à café][cannelle moulue]
		\ingredient{1/4}[c. à café][muscade moulue]
		...
		\ingredient{2-3}[c. à soupe][beurre][pour la cuisson]
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{1/4}[tasse][feuilles de basilic frais][hachées]
		\ingredient{2}[c. à soupe][huile d'olive extra vierge]
		\ingredient{1}[c. à soupe][vinaigre balsamique]
		\ingredient{}[][Sel et poivre noir][au goût]

		\ingredientsection{Pain}
		\ingredient{1}[][baguette française\note{Une baguette de la veille fonctionne mieux pour la bruschetta car elle grille plus uniformément et a une meilleure texture.}][tranchée en diagonale]
		\ingredient{2}[c. à soupe][huile d'olive]
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{2}[c. à soupe][jus de citron frais]
		\ingredient{1}[c. à café][moutarde de Dijon]
		\ingredient{1}[c. à café][sauce Worcestershire]
		\ingredient{1}[tasse][mayonnaise]
		\ingredient{1/2}[tasse][parmesan fraîchement râpé\note{Pour la meilleure saveur, utilisez du parmesan fraîchement râpé. La vinaigrette peut être préparée jusqu'à 3 jours à l'avance et conservée au réfrigérateur.}]
		\ingredient{}[][Sel et poivre noir]

		\ingredientsection{Salade}
		\ingredient{2}[][grosses têtes de laitue romaine][hachées]
		\ingredient{1}[tasse][croûtons maison ou du commerce]
		\ingredient{1/2}[tasse][copeaux de parmesan]
...
\recipe{%
	...
	vegetarian = {oui},
	ingredients = {
		\ingredient{4}[][grosses tomates mûres\note{Utilisez les meilleures tomates que vous pouvez trouver—les variétés anciennes ou mûries sur pied fonctionnent à merveille.}][tranchées de 6 mm d'épaisseur]
		\ingredient{450}[g][fromage mozzarella frais\note{La mozzarella di bufala fraîche est traditionnelle et fortement recommandée pour la saveur la plus authentique.}][tranché de 6 mm d'épaisseur]
		\ingredient{1}[tasse][feuilles de basilic frais]
		\ingredient{3}[c. à soupe][huile d'olive extra vierge]
		\ingredient{2}[c. à soupe][vinaigre balsamique ou glaçage]
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{1,4}[kg][ailes de poulet\note{Pour des ailes extra croustillantes, séchez-les complètement avant l'assaisonnement. Servir avec des bâtonnets de céleri et une vinaigrette au fromage bleu ou ranch.}][séparées aux articulations, bouts retirés]
		\ingredient{1}[c. à soupe][bicarbonate de soude]
		\ingredient{1}[c. à café][sel]
		\ingredient{1}[c. à café][poudre d'ail]
		\ingredient{1/2}[c. à café][poivre noir]

		\ingredientsection{Sauce Buffalo}
		\ingredient{1/2}[tasse][sauce piquante (Frank's RedHot de préférence)]
		\ingredient{1/3}[tasse][beurre non salé][fondu]
		\ingredient{1}[c. à soupe][vinaigre blanc]
		\ingredient{1/4}[c. à café][sauce Worcestershire]
		\ingredient{1/4}[c. à café][poivre de Cayenne][(optionnel, pour plus de piquant)]
	},
	instructions ={
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{1}[c. à soupe][huile d'olive]
		\ingredient{20}[g][beurre]
		\ingredient{2}[][oignons bruns][coupés en deux, finement hachés]
		\ingredient{2}[][gousses d'ail][écrasées]
		\ingredient{500}[g][viande de bœuf hachée]
		\ingredient{145}[g][pâte de tomate (1/2 tasse)]
		\ingredient{250}[mL][vin rouge sec (1 tasse)]
		\ingredient{2}[x 400g][boîtes de tomates en dés]
		\ingredient{1}[c. à soupe][origan séché]
		\ingredient{3}[][feuilles de laurier séchées]
		\ingredient{}[][Sel et poivre noir fraîchement moulu]
		\ingredient{1/3}[tasse][persil frais][légèrement tassé, grossièrement haché]

		\ingredientsection{Service}
		\ingredient{375}[g][spaghetti fins séchés]
		\ingredient{80}[g][parmesan][pour servir]
	},
	instructions ={
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{1/4}[tasse][huile d'olive]
		\ingredient{3}[c. à soupe][jus de citron frais]
		\ingredient{2}[][gousses d'ail][hachées]
		\ingredient{1}[c. à soupe][aneth frais][haché]
		\ingredient{1}[c. à soupe][persil frais][haché]
		\ingredient{1}[c. à café][zeste de citron]
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{2}[c. à soupe][huile de coco]
		\ingredient{3-4}[c. à soupe][pâte de curry rouge thaï\note{La pâte de curry rouge thaï varie en niveau de piquant selon les marques. Commencez avec moins et ajoutez-en au goût.}]
		\ingredient{1}[boîte][lait de coco (400 mL)]
		\ingredient{1}[tasse][bouillon de légumes]
		\ingredient{2}[c. à soupe][sauce soja ou tamari]
		\ingredient{1}[c. à soupe][sucre brun ou sirop d'érable]
		\ingredient{2}[][poivrons][tranchés]
		\ingredient{1}[][aubergine moyenne][coupée en cubes]
		\ingredient{1}[tasse][haricots verts][parés]
		\ingredient{1}[tasse][pousses de bambou]
		\ingredient{1}[tasse][feuilles de basilic thaï]
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{2}[tasses][farine tout usage]
		\ingredient{2}[tasses][sucre blanc]
		\ingredient{3/4}[tasse][poudre de cacao non sucrée]
		\ingredient{2}[c. à café][bicarbonate de soude]
		\ingredient{1}[c. à café][levure chimique]
		\ingredient{1}[c. à café][sel]
		...
		\ingredient{1}[tasse][babeurre]
		\ingredient{1}[tasse][café noir fort][chaud]
		\ingredient{1/2}[tasse][huile végétale]
		\ingredient{2}[c. à café][extrait de vanille]

		\ingredientsection{Glaçage}
		\ingredient{1}[tasse][beurre non salé (2 bâtonnets)\note{Les couches de gâteau peuvent être préparées la veille, enveloppées dans du film plastique et conservées à température ambiante.}][ramolli]
		\ingredient{3 1/2}[tasses][sucre glace]
		\ingredient{1/2}[tasse][poudre de cacao non sucrée]
		\ingredient{1/2}[tasse][crème épaisse]
		\ingredient{2}[c. à café][extrait de vanille]
		\ingredient{1/4}[c. à café][sel]
	},
	instructions ={
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{3/4}[tasse][sucre blanc]
		\ingredient{1 1/3}[tasse][fromage mascarpone][à température ambiante]
		\ingredient{2}[tasses][crème épaisse][froide]

		\ingredientsection{Couche de Café}
		\ingredient{2}[tasses][espresso fort ou de café\note{Utilisez un espresso ou un café fort de haute qualité pour la meilleure saveur.}][refroidi]
		\ingredient{3}[c. à soupe][liqueur de café][(optionnel)]
		\ingredient{40-48}[][biscuits à la cuillère (savoiardi)]

		\ingredientsection{Garniture}
		\ingredient{2}[c. à soupe][poudre de cacao non sucrée]
...
\recipe{%
	...
	ingredients = {
		...
		\ingredient{2 1/2}[tasses][farine tout usage]
		\ingredient{1}[c. à café][sel]
		\ingredient{1}[c. à soupe][sucre blanc]
		\ingredient{1}[tasse][beurre non salé froid (2 bâtonnets)\note{Pour une croûte ultra feuilletée, assurez-vous que tous les ingrédients sont froids et manipulez la pâte le moins possible.}][coupé en cubes]
		\ingredient{6-8}[c. à soupe][eau glacée]

		\ingredientsection{Garniture aux Pommes}
		\ingredient{6-7}[tasses][pommes tranchées (mélange de Granny Smith et Honeycrisp)]
		\ingredient{2/3}[tasse][sucre blanc]
		\ingredient{1/4}[tasse][farine tout usage]
		\ingredient{1}[c. à café][cannelle moulue]
		\ingredient{1/4}[c. à café][muscade moulue]
		\ingredient{1/4}[c. à café][sel]
		\ingredient{2}[c. à soupe][jus de citron]
		\ingredient{2}[c. à soupe][beurre][coupé en petits morceaux]

		\ingredientsection{Garniture}
		\ingredient{1}[][œuf][battu (pour dorure)]
		\ingredient{1}[c. à soupe][sucre cristallisé]
	},
	instructions ={
...
\recipe{%
	...
	ingredients = {
		\ingredient{2 1/4}[tasses][farine tout usage]
		\ingredient{1}[c. à café][bicarbonate de soude]
		\ingredient{1}[c. à café][sel]
		\ingredient{1}[tasse][beurre (2 bâtons)][ramolli]
		\ingredient{3/4}[tasse][sucre blanc]
		\ingredient{3/4}[tasse][cassonade][tassée]
		\ingredient{2}[][gros œufs]
		\ingredient{2}[c. à café][extrait de vanille]
		\ingredient{2}[tasses][pépites de chocolat]
	},
	instructions ={
...
\recipe{%
	...
	ingredients = {
		\ingredient{1}[tasse][jus de citron frais (environ 6-8 citrons)]
		\ingredient{1}[tasse][sucre blanc]
		\ingredient{5}[tasses][eau froide]
		\ingredient{}[][Glaçons]
